...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  private getRoomUserList(roomId: string): { id: string; username: string; avatar?: string }[] {
  ...
  handleCreateRoom(
    ...
    const roomId = this.rooms.has(id) ? `room-${Date.now()}-${Math.random().toString(36).slice(2, 6)}` : id;
  ...
  handleDeleteRoom(
    ...
    const roomId = (data?.roomId || '').trim();
    if (roomId === GENERAL_ID) return { success: false, error: 'Cannot delete general' };

    const room = this.rooms.get(roomId);
    if (!room) return { success: false, error: 'Room not found' };
    if (room.creatorId !== client.id) return { success: false, error: 'Only creator can delete' };

    const inRoom = Array.from(this.users.entries()).filter(([, u]) => u.roomId === roomId);
    this.rooms.delete(roomId);
    this.server.emit('rooms_updated', { rooms: this.getRoomsList() });
    this.server.to(roomId).emit('room_deleted', { roomId, newRoomId: GENERAL_ID });
  ...
  handleTypingStart(@ConnectedSocket() client: Socket) {
    ...
    user.typing = true;
    client.broadcast.to(user.roomId).emit('typing', { usernames: this.getTypingUsernames(user.roomId) });
  }
  ...
  handleUpdateUsername(
    ...
    const user = this.users.get(client.id);
    if (!user?.username || !user.roomId) return { success: false, error: 'Join first' };
