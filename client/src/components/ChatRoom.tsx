...
/** True if text is only emoji (and optional spaces), so we can show it bigger. */
...
export function ChatRoom({
  ...
  return (
    <div className="flex flex-1 flex-col min-w-0 min-h-0 bg-slate-50 dark:bg-slate-900">
      ...
      >
        <div className="relative flex items-center gap-2 rounded-[1.25rem] border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700/80 shadow-[0_1px_2px_rgba(0,0,0,0.04)] dark:shadow-[0_1px_2px_rgba(0,0,0,0.2)] focus-within:border-blue-400 dark:focus-within:border-blue-500 focus-within:shadow-[0_0_0_2px_rgba(59,130,246,0.2)] dark:focus-within:shadow-[0_0_0_2px_rgba(96,165,250,0.25)] transition-[border-color,box-shadow] duration-150">
          <div className="relative shrink-0" ref={emojiPickerRef}>
            <button
              type="button"
              onClick={() => setEmojiPickerOpen((open) => !open)}
              className={`shrink-0 flex items-center justify-center w-9 h-9 ml-1 rounded-full text-slate-400 hover:text-slate-600 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-600/80 transition-colors ${emojiPickerOpen ? "bg-slate-100 dark:bg-slate-600/80 text-slate-600 dark:text-slate-200" : ""}`}
            ...
            {emojiPickerOpen && (
              <div className="absolute bottom-full left-0 mb-2 z-50 w-[320px] sm:w-[352px] max-h-[280px] overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 shadow-xl dark:shadow-xl dark:shadow-black/30">
                <div className="p-2 border-b border-slate-100 dark:border-slate-700">
                  <p className="text-xs font-medium text-slate-500 dark:text-slate-400 px-1">
                    Pick an emoji
                  </p>
                </div>
                <div className="p-2 overflow-y-auto max-h-[232px] grid grid-cols-10 gap-0.5">
                  {EMOJI_LIST.map((emoji, i) => (
                    <button
                      key={`${emoji}-${i}`}
                      type="button"
                      onClick={() => insertEmoji(emoji)}
                      className="w-8 h-8 flex items-center justify-center rounded-lg text-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                      title={emoji}
                    >
                      {emoji}
                    </button>
